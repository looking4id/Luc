...
export const IterationList = ({ sprintId }: { sprintId: string }) => {
  // Mock data: filter all tasks for this "sprint"
        ...
        <div className="flex items-center gap-3">
          <div className="relative">
            <input 
              type="text" 
              placeholder="搜索工作项..." 
              className="pl-8 pr-4 py-1.5 text-xs border border-slate-300 rounded bg-white w-64 focus:outline-none focus:border-blue-500"
            />
            <Search size={14} className="absolute left-2.5 top-1/2 -translate-y-1/2 text-slate-400" />
          </div>
          <button className="flex items-center gap-1 px-3 py-1.5 bg-white border border-slate-200 rounded text-xs text-slate-600 hover:bg-slate-50 transition-colors">
            <Filter size={14} /> 筛选
          </button>
        </div>
        <div className="flex items-center gap-2">
          <button className="px-3 py-1.5 bg-blue-600 text-white rounded text-xs hover:bg-blue-700 transition-colors">规划工作项</button>
          ...
          <tbody>
            {allTasks.slice(0, 10).map((task, idx) => (
              <tr key={task.id} className="border-b border-slate-50 hover:bg-slate-50 group cursor-pointer transition-colors">
                <td className="py-3 px-6 text-xs font-mono text-slate-400">{task.displayId}</td>
                <td className="py-3 px-4 font-medium text-slate-700 text-sm group-hover:text-blue-600">{task.title}</td>
                <td className="py-3 px-4"><StatusBadge status={idx % 2 === 0 ? '进行中' : '已完成'} /></td>
                <td className="py-3 px-4 text-xs text-slate-600">
                  <div className="flex items-center gap-2">
                    <div className={`w-5 h-5 rounded-full ${task.assignee.avatarColor} text-white flex items-center justify-center text-[10px] font-bold`}>
                ...
                <td className="py-3 px-4 text-xs text-slate-400 font-mono">{task.dueDate}</td>
              </tr>
            ))}
          </tbody>
        </table>
